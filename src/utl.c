...
#define DSC_PPS_SIZE 128  /* matches PPS_SIZE in dsc_types.h */
...
pic_t *readbmp(FILE *fp)
{
    ...
    // Read BMP header (54 bytes for BITMAPFILEHEADER + BITMAPINFOHEADER)
    ...
    // Parse header
    data_offset = bmp_header[10] | (bmp_header[11] << 8) | (bmp_header[12] << 16) | (bmp_header[13] << 24);
    ...
    compression = bmp_header[30] | (bmp_header[31] << 8) | (bmp_header[32] << 16) | (bmp_header[33] << 24);
    ...
    // Create picture structure
    p = pcreate(FRAME, RGB, YUV_444, width, height);
    p->bits = 8;  // BMP is typically 8-bit per channel

    // Skip to pixel data
    fseek(fp, data_offset, SEEK_SET);

    // Calculate padding per row (rows are 4-byte aligned)
    padding = (4 - (width * 3) % 4) % 4;

    // Allocate row buffer
    row_data = (unsigned char *)malloc(width * 3);

    // Read pixel data (BMP stores rows from bottom to top)
    for (i = height - 1; i >= 0; i--) {
        if (fread(row_data, 1, width * 3, fp) != width * 3) {
            free(row_data);
            pdestroy(p);
            return NULL;
        }
        ...
        // Convert BGR to RGB
        for (j = 0; j < width; j++) {
            p->data.rgb.b[i][j] = row_data[j * 3];     // Blue
            p->data.rgb.g[i][j] = row_data[j * 3 + 1]; // Green
            p->data.rgb.r[i][j] = row_data[j * 3 + 2]; // Red
        }
    }
...
int bmp_read_with_dsc(char *fname, pic_t **pic, unsigned char **dsc_data, int *dsc_size, int *bmp_w, int *bmp_h)
{
    FILE *fp;
    unsigned char bmp_header[54];
    int width, height, bpp, compression;
    int file_size, data_offset, embedded_size;
    ...
    // Parse header
    file_size = bmp_header[2] | (bmp_header[3] << 8) | (bmp_header[4] << 16) | (bmp_header[5] << 24);
    data_offset = bmp_header[10] | (bmp_header[11] << 8) | (bmp_header[12] << 16) | (bmp_header[13] << 24);
    width = bmp_header[18] | (bmp_header[19] << 8) | (bmp_header[20] << 16) | (bmp_header[21] << 24);
    height = bmp_header[22] | (bmp_header[23] << 8) | (bmp_header[24] << 16) | (bmp_header[25] << 24);
    bpp = bmp_header[28] | (bmp_header[29] << 8);
    ...
    // Return BMP dimensions to caller
    if (bmp_w) *bmp_w = width;
    if (bmp_h) *bmp_h = height;

    // Create picture structure
    *pic = pcreate(FRAME, RGB, YUV_444, width, height);
    (*pic)->bits = 8;

    // Read everything after BMP header as DSC payload
    embedded_size = file_size - 54;
    if (embedded_size <= 4 + DSC_PPS_SIZE) {
        fclose(fp);
        *dsc_data = NULL;
        *dsc_size = 0;
        return 0;
    ...
    // Read and verify DSCF signature
    if (fread(signature, 1, 4, fp) != 4 ||
        signature[0] != 'D' || signature[1] != 'S' ||
    ...
    // Read full DSC payload (DSCF + PPS + compressed data)
    *dsc_data = (unsigned char *)malloc(embedded_size);
    if (*dsc_data) {
        memcpy(*dsc_data, signature, 4);
        if (fread(*dsc_data + 4, 1, embedded_size - 4, fp) == embedded_size - 4) {
            *dsc_size = embedded_size;
        } else {
            free(*dsc_data);
            *dsc_data = NULL;
            *dsc_size = 0;
...
int bmp_read(char *fname, pic_t **pic)
{
    FILE *fp;
...
void writebmp(FILE *fp, pic_t *p)
{
    int i, j;
    unsigned char pad[4] = {0, 0, 0, 0};
    int padding = (4 - (p->w * 3) % 4) % 4;

    // BITMAPFILEHEADER
    fputc('B', fp); fputc('M', fp);  // bfType
    int fileSize = 54 + (p->w * 3 + padding) * p->h;
    fwrite(&fileSize, 4, 1, fp);    // bfSize
    fputc(0, fp); fputc(0, fp);     // bfReserved1
    fputc(0, fp); fputc(0, fp);     // bfReserved2
    int dataOffset = 54;
    fwrite(&dataOffset, 4, 1, fp);  // bfOffBits

    // BITMAPINFOHEADER
    int headerSize = 40;
    fwrite(&headerSize, 4, 1, fp);  // biSize
    fwrite(&p->w, 4, 1, fp);        // biWidth
    fwrite(&p->h, 4, 1, fp);        // biHeight
    short planes = 1;
    fwrite(&planes, 2, 1, fp);      // biPlanes
    short bitCount = 24;
    fwrite(&bitCount, 2, 1, fp);    // biBitCount
    int compression = 0;
    fwrite(&compression, 4, 1, fp); // biCompression
    int imageSize = (p->w * 3 + padding) * p->h;
    fwrite(&imageSize, 4, 1, fp);   // biSizeImage
    int xPels = 0;
    fwrite(&xPels, 4, 1, fp);       // biXPelsPerMeter
    int yPels = 0;
    fwrite(&yPels, 4, 1, fp);       // biYPelsPerMeter
    int clrUsed = 0;
    fwrite(&clrUsed, 4, 1, fp);     // biClrUsed
    int clrImportant = 0;
    fwrite(&clrImportant, 4, 1, fp); // biClrImportant
    ...
    for (i = p->h - 1; i >= 0; i--)
    {
        for (j = 0; j < p->w; j++)
        {
            ...
            if (p->bits > 8)
            {
                r = (unsigned char)(p->data.rgb.r[i][j] >> 8);
                g = (unsigned char)(p->data.rgb.g[i][j] >> 8);
                b = (unsigned char)(p->data.rgb.b[i][j] >> 8);
            }
            else
            {
                r = (unsigned char)p->data.rgb.r[i][j];
                g = (unsigned char)p->data.rgb.g[i][j];
                b = (unsigned char)p->data.rgb.b[i][j];
            }
            fputc(b, fp);  // Blue
            fputc(g, fp);  // Green
            fputc(r, fp);  // Red
...
void writebmp_with_dsc(FILE *fp, pic_t *p, unsigned char *dsc_data, int dsc_size, int bmp_w, int bmp_h)
{
    int fileSize = 54 + dsc_size;
    ...
    fputc(0, fp); fputc(0, fp);     // bfReserved2
    int dataOffset = 54;
    fwrite(&dataOffset, 4, 1, fp);  // bfOffBits (pixel data starts at 54)

    // BITMAPINFOHEADER
    int headerSize = 40;
    fwrite(&headerSize, 4, 1, fp);  // biSize
    fwrite(&bmp_w, 4, 1, fp);       // biWidth = slice width
    fwrite(&bmp_h, 4, 1, fp);       // biHeight = slice height or full height
    short planes = 1;
    fwrite(&planes, 2, 1, fp);      // biPlanes
    short bitCount = 24;
    fwrite(&bitCount, 2, 1, fp);    // biBitCount
    int compression = 0;
    fwrite(&compression, 4, 1, fp); // biCompression
    int imageSize = dsc_size;       // biSizeImage = DSC data size
    ...
    // Write raw DSC compressed data after the 54-byte header
    if (dsc_data && dsc_size > 0) {
        fwrite(dsc_data, 1, dsc_size, fp);
    }
}


int bmp_write(char *fname, pic_t *pic)
{
	FILE *fp;
	pic_t *temp_pic, *temp_pic2;
...
int bmp_write_with_dsc(char *fname, pic_t *pic, unsigned char *dsc_data, int dsc_size, int bmp_w, int bmp_h)
{
	FILE *fp;
	pic_t *temp_pic, *temp_pic2;
	...
	if(pic->chroma == YUV_420)
	{
		...
		temp_pic->bits = pic->bits;
		yuv2rgb(temp_pic2, temp_pic);
		writebmp_with_dsc(fp, temp_pic, dsc_data, dsc_size, bmp_w, bmp_h);
		pdestroy(temp_pic);
		pdestroy(temp_pic2);
	}
	else if(pic->chroma == YUV_422)
	...
	else if(pic->color != RGB)
	{
		...
		writebmp_with_dsc(fp, pic, dsc_data, dsc_size, bmp_w, bmp_h);
