...
/* Use setjmp/longjmp to catch all exit() calls so the GUI process survives */
#include <setjmp.h>
static jmp_buf g_codec_jmp;
static int g_codec_exit_code = 0;
#define exit(code) longjmp(g_codec_jmp, (code) == 0 ? 1 : (code))
...
static int bmpFileOutput;
static int bmpDscOutput;
static int yuvFileOutput;
static int yuvFileFormat;
static int printPps;

// DSC data collection for BMP embedding
static unsigned char *dsc_buffer = NULL;
static int dsc_buffer_size = 0;
static int dsc_buffer_capacity = 0;
static int dsc_buffer_pos = 0;

static void init_dsc_buffer() {
    dsc_buffer_size = 0;
    dsc_buffer_capacity = 1024 * 1024; // Start with 1MB
    dsc_buffer = (unsigned char *)malloc(dsc_buffer_capacity);
    if (!dsc_buffer) { UErr("Failed to allocate DSC buffer\n"); }
    dsc_buffer_pos = 0;
}

static void free_dsc_buffer() {
    if (dsc_buffer) {
        free(dsc_buffer);
        dsc_buffer = NULL;
    }
    dsc_buffer_size = 0;
    dsc_buffer_capacity = 0;
    dsc_buffer_pos = 0;
}

static void append_dsc_data(unsigned char *data, int size) {
    if (dsc_buffer_size + size > dsc_buffer_capacity) {
        // Expand buffer
        int new_capacity = (dsc_buffer_size + size) * 2;
        unsigned char *new_buf = (unsigned char *)realloc(dsc_buffer, new_capacity);
        if (!new_buf) { UErr("Failed to realloc DSC buffer\n"); }
        dsc_buffer = new_buf;
        dsc_buffer_capacity = new_capacity;
    }
    memcpy(dsc_buffer + dsc_buffer_size, data, size);
    dsc_buffer_size += size;
}
...
static  cmdarg_t cmd_args[] = {
	...
	{ PARG,  &bmpFileOutput,      "BMP_FILE_OUTPUT",      "-bmp",  0,  0},    // output BMP files
	{ PARG,  &bmpDscOutput,       "BMP_DSC_OUTPUT",       "-bmpdsc", 0, 0},   // embed DSC data in BMP files
	...
	{ PARG,  &dscVersionMinor,    "DSC_VERSION_MINOR", "-dvm", 0, 0},      // DSC minor version
	 ...
	 {NARG,  &printPps,           "PRINT_PPS",             "-P"      , 0, 0}, // print out PPS
	 {NARG,  &help,               "",                     "-help"   , 0, 0}, // video format
     {SARG,   filepath,           "INCLUDE",              "-F"      , 0, 0}, // Cconfig file
     {SARG,   option,             "",                     "-O"      , 0, 0}, // key/value pair
     {SARG,   fn_i,               "SRC_LIST",              ""        , 0, 0}, // Input file name (list)
     {SARG,   srcDir,             "SRC_DIR",               ""        , 0, 0}, // Input directory (auto-scan)
...
/* Scan directory for image files and write to a temp list file */
static char temp_list_file[PATH_MAX+1] = "";

static void scan_directory_for_images(const char *dir_path)
{
    FILE *fp;
#ifdef _WIN32
    char search_path[PATH_MAX+10];
    ...
    snprintf(temp_list_file, sizeof(temp_list_file), "%s\\__gui_temp_list__.txt", dir_path);
    if (NULL == (fp = fopen(temp_list_file, "wt"))) {
        fprintf(stderr, "Cannot create temp list file\n");
        exit(1);
    }
    ...
    do {
        ...
        if (ext && (!strcmp(ext+1, "dpx") || !strcmp(ext+1, "DPX") ||
                    !strcmp(ext+1, "ppm") || !strcmp(ext+1, "PPM") ||
                    !strcmp(ext+1, "yuv") || !strcmp(ext+1, "YUV") ||
                    !strcmp(ext+1, "bmp") || !strcmp(ext+1, "BMP"))) {
            fprintf(fp, "%s\\%s\n", dir_path, name);
        }
...
#endif
    fclose(fp);
    /* Now set fn_i to point to the temp list file */
    strncpy(fn_i, temp_list_file, PATH_MAX);
}
...
static int parse_cfgfile (char* fn, cmdarg_t *cmdargs)
{
    ...
    {
        char cfg_fname[PATH_MAX+1];
        strncpy(cfg_fname, fn, PATH_MAX);
        cfg_fname[PATH_MAX] = '\0';
        fn[0] = '\0';
        ...
        if (!feof(fd))
        {
            PErr("while reading from file '%s'", cfg_fname);
        }
    }
    fclose(fd);
    return 0;
...
void write_dsc_data(unsigned char **bit_buffer, int nbytes, FILE *fp, int vbr_enable, int slices_per_line, int slice_height, int **sizes)
{
	...
	if (!current_idx) { UErr("Failed to allocate current_idx\n"); }
...
void write_dsc_data_to_buffer(unsigned char **bit_buffer, int nbytes, int vbr_enable, int slices_per_line, int slice_height, int **sizes)
{
	int i;
	int slice_x, slice_y;
	int *current_idx;
	...
	for (slice_y = 0; slice_y < slice_height; ++slice_y)
	{
		for (slice_x = 0; slice_x < slices_per_line; ++slice_x)
		{
			...
			{
				nbytes = sizes[slice_x][slice_y];
				temp_buf[0] = (nbytes>>8) & 0xff;
				temp_buf[1] = nbytes & 0xff;
				append_dsc_data(temp_buf, 2);
			}
			append_dsc_data(&bit_buffer[slice_x][current_idx[slice_x]], nbytes);
			current_idx[slice_x] += nbytes;
		}
	}
	free(current_idx);
...
int read_dsc_data_from_buffer(unsigned char **bit_buffer, int nbytes, unsigned char *buffer, int *buffer_pos, int buffer_size, int vbr_enable, int slices_per_line, int slice_height)
{
	int i, slice_y, slice_x;
	int *current_idx, total_bytes = 0;
	...
	for (slice_y = 0; slice_y < slice_height; ++slice_y)
	{
		for (slice_x = 0; slice_x < slices_per_line; ++slice_x)
		{
			if (vbr_enable)
			{
				if (*buffer_pos + 2 > buffer_size) return -1; // Buffer overflow
				nbytes = 0;
				for(i=0; i<2; ++i)
					nbytes = (nbytes<<8) | (buffer[(*buffer_pos)++] & 0xff);
			}
			if (*buffer_pos + nbytes > buffer_size) return -1; // Buffer overflow
			for(i=0; i<nbytes; ++i)
				bit_buffer[slice_x][i+current_idx[slice_x]] = buffer[(*buffer_pos)++] & 0xff;
			current_idx[slice_x] += nbytes;
			total_bytes += nbytes;
		}
	}
...
#endif
{
	...
	/* Use setjmp to catch all exit() calls from the codec */
	int jmp_rc = setjmp(g_codec_jmp);
	if (jmp_rc != 0) {
		/* We jumped here from an exit() call */
		return jmp_rc;
	}

	printf("Display Stream Compression (DSC) reference model version 1.48\n");
	...
	if (!rcOffset || !rcMinQp || !rcMaxQp || !rcBufThresh) { UErr("Failed to allocate RC arrays\n"); }
	...
	/* If SRC_DIR is specified but no list file, scan directory */
	if (srcDir[0] != '\0' && fn_i[0] == '\0') {
		scan_directory_for_images(srcDir);
	}

	/* At least one input source is required */
	if (fn_i[0] == '\0')
	{
		fprintf(stderr, "No input specified. Use SRC_LIST (image list file) or SRC_DIR (directory)\n");
		exit(1);
	}
	...
	while ((strlen(infname)>0) || !feof(list_fp) || multiFrameYuvFile)
	{
		...
		else if (!strcmp(extension, "bmp") || !strcmp(extension, "BMP"))
		{
			if (useYuvInput)
			{
				printf("BMP format is RGB only, converting source to YUV\n");
			}
			
			if (bmp_read(infname, &ip))
			{
				if (function == 2)
					printf("Could not read original image for decode, PSNR will not be computed\n");
				else
				{
					fprintf(stderr, "Error read BMP file %s\n", infname);
					exit(1);
				}
			}
		}
		...
		if ((function == 1) || (function == 0))
		{
			...
			{
				if (bmpDscOutput) {
					// Initialize DSC buffer for embedding in BMP
					init_dsc_buffer();
					// Write DSC signature and PPS to buffer
					append_dsc_data((unsigned char*)"DSCF", 4);
					write_pps(pps, &dsc_codec);
					append_dsc_data(pps, PPS_SIZE);
				} else {
#ifdef WIN32
		...
		{
			// Check if input is BMP with embedded DSC data
			if (!strcmp(extension, "bmp") || !strcmp(extension, "BMP")) {
				unsigned char *embedded_dsc_data = NULL;
				int embedded_dsc_size = 0;
				
				if (bmp_read_with_dsc(infname, &ip, &embedded_dsc_data, &embedded_dsc_size, NULL, NULL)) {
					if (function == 2)
						printf("Could not read BMP file for decode, PSNR will not be computed\n");
					else {
						fprintf(stderr, "Error read BMP file %s\n", infname);
						exit(1);
					}
				...
				if (embedded_dsc_data && embedded_dsc_size > 0) {
					// Use embedded DSC data
					if (embedded_dsc_data[0] != 'D' || embedded_dsc_data[1] != 'S' || 
						embedded_dsc_data[2] != 'C' || embedded_dsc_data[3] != 'F') {
						UErr("DSC data in BMP file has invalid magic number\n");
					}
					for (i=0; i<PPS_SIZE; ++i)
						pps[i] = embedded_dsc_data[4 + i];
					parse_pps(pps, &dsc_codec);
					
					// Create a memory buffer for DSC data
					dsc_buffer = embedded_dsc_data;
					dsc_buffer_size = embedded_dsc_size;
					dsc_buffer_capacity = embedded_dsc_size;
					dsc_buffer_pos = 4 + PPS_SIZE; // Skip signature and PPS
					bits_fp = NULL; // No file, using memory buffer
				} else {
					// Fall back to separate DSC file
					goto fallback_dsc_file;
				}
			} else {
fallback_dsc_file:
#ifdef WIN32
		...
		if (!buf) { UErr("Failed to allocate slice buffer array\n"); }
		for (i=0; i<slices_per_line; ++i) {
			buf[i] = (unsigned char *)malloc(bufsize);
			if (!buf[i]) { UErr("Failed to allocate slice buffer\n"); }
		}

		if(dsc_codec.native_420)
			op_dsc = (pic_t *)pcreate(FRAME, ip ? ip->color : RGB, YUV_444, dsc_codec.pic_width/2, dsc_codec.pic_height);
		...
		if (dsc_codec.convert_rgb)
		{
			...
			if (!temp_pic) { UErr("Failed to allocate temp_pic array\n"); }
		...
		if (!chunk_sizes) { UErr("Failed to allocate chunk_sizes array\n"); }
		for(i=0; i<slices_per_line; ++i)
		...
		for (ys = 0; ys < dsc_codec.pic_height; ys+=sliceh)
		{
			if(function == 2) {
				if (bits_fp) {
					...
					read_dsc_data_from_buffer(buf, dsc_codec.chunk_size, dsc_buffer, &dsc_buffer_pos, dsc_buffer_size, dsc_codec.vbr_enable, slices_per_line, dsc_codec.slice_height);
				}
			}
			...
			if(function == 1) {
				if (bmpDscOutput) {
					write_dsc_data_to_buffer(buf, dsc_codec.chunk_size, dsc_codec.vbr_enable, slices_per_line, dsc_codec.slice_height, chunk_sizes);
				} else {
		...
		for(i=0; i<slices_per_line; ++i)
			free(chunk_sizes[i]);
		free(chunk_sizes);
		if (function != 0 && bits_fp)
			fclose(bits_fp);

		// Free DSC buffer will be done after output writes
		// if (bmpDscOutput || (function == 2 && !bits_fp)) {
		// 	free_dsc_buffer();
		// }
		...
		if (function!=1 || bmpFileOutput || bmpDscOutput || dpxFileOutput || ppmFileOutput || yuvFileOutput)  // Don't write if encode only unless output flags are set
		{
			if (yuvFileOutput)
			{
			strcpy(f, fn_o);
			...
			if (bmpFileOutput || bmpDscOutput)
			{
				...
				if (multiFrameYuvFile || frameNumber > 0)
					strcat(f, frmnumstr);
				strcat(f, ".out.bmp");
				if (bmpDscOutput && dsc_buffer && dsc_buffer_size > 4 + PPS_SIZE) {
					// BMP dimensions = slice width x slice height (or full height if no slice_height set)
					if (bmp_write_with_dsc(f, op_dsc, dsc_buffer, dsc_buffer_size, dsc_codec.slice_width, sliceh))
					{
						fprintf(stderr, "Error writing BMP file with DSC data %s\n", f);
						exit(1);
					}
				} else {
					if (bmp_write(f, op_dsc))
					{
		...
		// Decode mode (function==2) always outputs decoded BMP with "UN" prefix
		if (function == 2 && op_dsc)
		{
			strcpy(f, fn_o);
#ifdef WIN32
			strcat(f, "\\");
			...
			if (bmp_write(f, op_dsc))
			{
				fprintf(stderr, "Error writing decoded BMP file %s\n", f);
				exit(1);
			}
		}
		...
		// Free DSC buffer after all output writes are complete
		if (bmpDscOutput || (function == 2 && !bits_fp)) {
			free_dsc_buffer();
		}
